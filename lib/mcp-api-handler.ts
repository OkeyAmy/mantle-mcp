...
export function initializeMcpApiHandler(
  ...
  const maxDuration =
    vercelJson?.functions?.["api/**/*"]?.maxDuration || 60;
  const redisUrl = process.env.REDIS_URL || process.env.KV_URL;
  
  if (!redisUrl) {
    console.warn("REDIS_URL environment variable is not set. Some functionality may be limited.");
  }
  
  // Initialize Redis clients only if URL is available
  const redis = redisUrl ? createClient({ url: redisUrl }) : null;
  const redisPublisher = redisUrl ? createClient({ url: redisUrl }) : null;
  
  // Set up error handlers if Redis is available
  if (redis) {
    redis.on("error", (err) => {
  ...
  // Only connect to Redis if clients are available
  const redisPromise = redis && redisPublisher 
    ? Promise.all([redis.connect(), redisPublisher.connect()])
    : Promise.resolve();
